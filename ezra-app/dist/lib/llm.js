...
class LLMService {
    ...
    lastRequestTime = 0;
    MIN_REQUEST_INTERVAL = 4000; // 4 seconds between requests for free tier
    constructor() {
        ...
        this.liteModel = new google_genai_1.ChatGoogleGenerativeAI({
            model: "gemini-2.0-flash",
            apiKey: process.env.GOOGLE_API_KEY,
            temperature: 0.7,
            maxOutputTokens: 2048,
        ...
        try {
            this.advancedModel = new google_genai_1.ChatGoogleGenerativeAI({
        ...
        this.tokenizer = (0, js_tiktoken_1.getEncoding)("cl100k_base"); // GPT-4 encoding
    }
    /**
     * Rate-limited request execution to prevent 503 errors on free tier
     */
    async executeWithRateLimit(requestFn, retries = 3) {
     ...
     */
    logTokenUsage(label, promptText, responseText) {
     ...
     */
    async generateDistilledMasterPrompt(fullMasterPrompt) {
        console.log("🌀 Starting distilled Master Prompt generation...");
        const distillPromptTemplate = (0, prompts_2.readPromptFile)('distilledMasterPromptGenerator.md');
     ...
     */
    async updateFullMasterPrompt(originalFullMasterPrompt, originalDistilledPrompt, userEditedDistilledPrompt) {
        console.log("🔄 Starting full Master Prompt update from distilled edits...");
        const updaterPromptTemplate = (0, prompts_2.readPromptFile)('masterPromptUpdaterFromDistilled.md');
     ...
     */
    async generateStyleSummary(historicalEmails) {
        if (historicalEmails.length === 0) {
            return "No historical communication style data available.";
        }
        const styleAnalysisPrompt = prompts_1.PromptTemplate.fromTemplate((0, prompts_2.getStyleAnalysisPrompt)());
    ...
    async invokeIncomingScanner(emailData) {
        try {
            ...
            const scannerPrompt = (0, prompts_2.readPromptFile)('incomingEmailScannerPrompt.md');
            ...
            const responseText = await this.executeWithRateLimit(async () => {
                const response = await this.model.invoke([
                    new messages_1.SystemMessage(systemMessage),
                    new messages_1.HumanMessage(formattedPrompt)
                ]);
                return response.content;
            });
            // Parse JSON response
            ...
            const scannerOutput = JSON.parse(cleanedResponse);
     ...
     */
    async invokeStyleCompressor(masterPrompt, detailedStyleAnalysis, historicalEmails) {
        try {
            console.log('✂️ Compressing style analysis into efficient guide...');
            const styleCompressorPrompt = (0, prompts_2.readPromptFile)('styleCompressorPrompt.md');
...
• Request clarification if needed`;
     ...
     */
    async generateReply(masterPrompt, emailContext, styleSummary, contextualDraft) {
        console.log("🤖 Starting LLM reply generation...");
        const replyGenerationPrompt = prompts_1.PromptTemplate.fromTemplate((0, prompts_2.getReplyGenerationPrompt)());

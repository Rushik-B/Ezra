...
class GmailService {
            ...
            for (const email of emails) {
                try {
                    ...
                    if (!thread) {
                        ...
                        try {
                            thread = await prisma_1.prisma.thread.create({
                                data: {
                                    userId,
                                    subject: email.subject,
                            ...
                            if (threadError.code === 'P2002') {
                                thread = await prisma_1.prisma.thread.findFirst({
                                    where: {
                                        userId,
                                        subject: email.subject
                                    }
                    ...
                    await prisma_1.prisma.email.upsert({
                        where: {
     ...
     */
    async fetchContextualEmails(userId, query) {
        try {
            ...
            if (emails.length > 0) {
                ...
                const keywordMatches = emails.filter(email => query.keywords?.some(keyword => email.subject.toLowerCase().includes(keyword.toLowerCase()) ||
     ...
     */
    generateEmailContextSummary(emails) {
        if (emails.length === 0) {
